...
- Supabase 는 `React 라면 Localstorage 에 로그인 정보` 보관
  - localstorage 는 웹브라우저 종료해도 남아있어요.
  - localstorage 는 새로고침해도 남아있어요.
  - localstorage 는 유효기간이 없다.
- Supabase 는 `Next 라면 cookie 에 로그인 정보` 보관
  - cookie 는 웹브라우저 종료해도 남아있어요.
  - cookie 는 새로고침해도 남아있어요.
  - cookie 는 일정 기간만큼만 보관(유효기간 존재)
...
create(
  combine(initialState, (set, get) => ({
    actions: {
      setSesstion: (session: Session | null) => {
        // 하고싶은일
...
- local Storage 에 보관하지 않아도 웹브라우저가 가지고 있습니다.
