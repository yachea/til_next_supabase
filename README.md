...
- Context API
- Context API 를 응용한 라이브러리 : Redux, Recoil, `Zustand`7, Mobx, Joitai...
...
- zustand 는 객체 내부의 스테이트의 키명을 기분으로 명시만 하면됨.
- 키명을 명시하고 새로운 값만 작성해 주면됨.
- State 객체를 ... 즉, spred 할 필요없음.

- 단계 12 : set 을 사용시 함수형태도 지원함.
...
// 커스텀 훅을 리턴해준다. 아주 좋다.
const useCountStore = create<CountStoreType>((set, get) => {
  return {
    // state : 초기값
    count: 0,
    // action : state 변경
...
- `/src/app/counter/page.tsx 파일` 업데이트
