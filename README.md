...
- Auth provider > email > `Enable Email provider` 만 활성화
...
function SignUp() {
  return (
    <div className='flex flex-col gap-8'>
      <div className='text-xl font-bold text-center'>회원가입</div>
      <div className='flex flex-col gap-8'>
...
- 백엔드 연동 `API 가 완성`되면 이제 `관리 해줄 MUtation` 을 만든다.

### 5.3. 왜 Mutation 을 쓰라는 거지?

- 현재 글을 등록을 하는데
- 현재 등록중인 상태를 `isPending 개발자가 코딩으로 관리한다.`
- 현재 글이 등록 완료되었는지 `response 가 true이면 개발자가 코딩으로 관리한다.`
- 현재 글이 등록 실패했는지 `error 처리 개발자가 코딩으로 관리한다.`
- 현재 글이 등록 오류가 발생시 `error 개발자가 코딩으로 관리한다.`
- 각각의 상황에 맞는 함수들(이벤트에 따라서 실행할 콜백함수 등)을 직접 작성한다.
